...
async function loadComparison(){
  ...
  try{
    ...
    for(const d of info){
      ...
      const data=await api('/api/image?'+new URLSearchParams({dataset:d.id,pattern:choice.pattern,wavenumber:wn,kind,cnr_background_source:d.cnr_rois?.background_source||'manual',low:choice.low,high:choice.high,colorbar_zero:!!choice.zero[kind]}));if(current!==revision)return;
...
function attachComparisonProfile({card,img,overlay,data,dataset,choice,kind,wn,current}){
  ...
  function clearLine(){line.revision++;line.start=line.end=null;line.selecting=false;card.querySelector('.line-profile-plot')?.remove();draw();status.textContent='Choose Horizontal or Vertical, then select two points. Positions and profiles are independent for each folder; values are unaffected by display contrast.';}
  async function refresh(){
    const token=++line.revision;
    ...
    try{
      ...
      QCLLine.renderPlot(card,kind,dataset.name+' '+kind,dataset.has_gold,result);
  ...
  img.onpointerdown=e=>{
    if(!line.selecting||e.button!==0||current!==revision||datasets.some(d=>d.busy))return;
