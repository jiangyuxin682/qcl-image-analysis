...
function addSpectrumLabels(wrap,data){
  ...
  const canvas=wrap.querySelector('canvas'),holder=el('div');
  ...
  const scale=data.width/700,radius=3.5*scale,fontSize=11*scale;
  for(const peak of data.peaks||[]){
    const x=peak.ix+.5,y=peak.iy+.5,circle=document.createElementNS(ns,'circle');
    ...
    text.setAttribute('y',Math.max(fontSize,y-radius-2*scale));
...
function updateVideoTitle(){if(state.time)$('#time-settings').textContent=`${state.time.wavenumber} cm⁻¹ · ${state.time.kind==='baseline'?'Absorbance after baseline':'Absorbance before baseline'} · Contrast: ${state.time.low}–${state.time.high} percentiles · FPS: ${playbackFPS()}`;}
